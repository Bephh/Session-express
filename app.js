...
app.get('/', (req, res) => {
    ...
    if (req.session.usuario) {
        ...
        res.send(`Olá ${req.session.usuario}, Você visitou essa pagina ${req.session.visitas} Vezes.`);



    } else {
        res.send('Você visitou essa página 1 vez. Faça o login!')
    }
...
app.post('/login', (req, res)=>{
    ...
    if (username === 'nicolas' && password === '123') {
...
app.get('/perfil', (req, res) =>{
    if (res.session.usuario) {
        ...
        req.send('Você precisa fazer o login primeiro!')
    }
});
